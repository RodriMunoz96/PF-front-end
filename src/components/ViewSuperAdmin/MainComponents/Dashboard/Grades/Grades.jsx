...
import {
  ...
  getGradeById,
  removeGrade,
} from "../../../../../redux/actions/action-grades";

function Grades() {
  const dispatch = useDispatch();
  const { allGradesCopy, currentPage, gradesDetail } = useSelector(
    (state) => state
  );
              ...
              ? currentPageData.map((grade, index) => {
                  ...
                  return (
                    <tr className={`${style.row} ${rowClass}`} key={grade.id}>
                      <td>
                        <button onClick={() => openStatus(grade.id)}>⁝</button>
                      </td>
      ...
      {statusOpen ? (
        <div className={style.overlay}>
          <div className={style.container_status_modify}>
            ...
            <h2>
              ¿Desea {gradesDetail.state ? "desabilitar" : "habilitar"} el
              grado: {gradesDetail.gradename}?
            </h2>
            {
              gradesDetail.state && (
                <button
                  className={style.button_action}
                  onClick={() => banToGrade(gradesDetail.id)}
                >
                  Desabilitar grado
                </button>
              )
